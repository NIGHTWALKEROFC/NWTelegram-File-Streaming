...
//
// KEY CHANGE: _playFile() now uses two strategies:
//
// SMALL (audio, docs, video < 50 MB):
//   Navigate to PlayerScreen immediately with mode=download.
//   PlayerScreen shows a progress bar while downloadAndWait() runs,
//   then plays from file:// the moment download completes.
//   Zero proxy, zero MIME issues, zero range bugs.
//
// LARGE (video ≥ 50 MB):
//   Navigate immediately with mode=proxy.
//   PlayerScreen calls prepareFile() → proxy serves HTTP range requests
//   via readFilePart() → plays while downloading.
...
class _FilesScreenState extends State<FilesScreen>
  ...
  void _startLoading() {
    ...
    setState(() {
      _allFiles = []; _filtered = [];
      _isLoading = true; _hasData = false; _loadError = null;
    });
    _loadCacheSize();
    ...
    _fileSub = svc.streamAllMediaFiles(limitPerChat: 30).listen(
      (files) {
        if (!mounted) return;
        setState(() { _allFiles = files; _hasData = files.isNotEmpty; });
        _applyFilter();
      },
      onError: (e) {
        if (!mounted) return;
  ...
  Future<void> _loadCacheSize() async {
    try {
      ...
      int total = 0;
      await for (final e in filesDir.list(recursive: true)) {
        if (e is File) total += await e.length();
      }
      if (mounted) setState(() => _cacheSize = _fmtBytes(total));
    } catch (_) { if (mounted) setState(() => _cacheSize = ''); }
  }
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          ...
          'Files stay safe on Telegram servers.',
  ...
  void _applyFilter() {
    ...
    List<TelegramFile> r = List.of(_allFiles);
    switch (_tabIndex) {
      case 1: r = r.where((f) => f.isVideo).toList();    break;
      case 2: r = r.where((f) => f.isAudio).toList();    break;
      case 3: r = r.where((f) => f.isDocument).toList(); break;
    }
    if (q.isNotEmpty) {
      r = r.where((f) => f.name.toLowerCase().contains(q)).toList();
    }
    setState(() => _filtered = r);
  }

  // ── Play a file — TWO STRATEGIES ──────────────────────────────────────────
  ...
  Future<void> _playFile(TelegramFile file) async {
    ...
    // Ensure proxy server is running (needed for large videos)
    if (!streamSvc.isRunning) await streamSvc.startServer(telegramSvc);

    // Pick quality for multi-quality videos
    VideoQuality? initialQuality;
    if (file.isVideo && file.qualities.length > 1) {
      initialQuality = file.qualities.last; // lowest = fastest
    } else if (file.isVideo && file.qualities.isNotEmpty) {
    ...
    // Unknown size (0) → assume small so we try download-first
    final bool isLargeVideo =
        file.isVideo && fileSize >= kSmallFileThreshold;

    if (!mounted) return;

    if (isLargeVideo) {
      // LARGE VIDEO → proxy (plays while downloading)
      Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => PlayerScreen(
          file:           file,
          initialQuality: initialQuality,
          streamUrl:      streamSvc.streamUrl,
          playMode:       PlayMode.proxy,
          fileId:         fileId,
          fileSize:       fileSize,
        ),
      ));
    } else {
      // AUDIO / SMALL VIDEO / DOCUMENT → download-first, play from file://
      Navigator.of(context).push(MaterialPageRoute(
        builder: (_) => PlayerScreen(
          file:           file,
          initialQuality: initialQuality,
          streamUrl:      streamSvc.streamUrl, // not used in download mode
          playMode:       PlayMode.download,
          fileId:         fileId,
          fileSize:       fileSize,
        ),
      ));
      ...
      body: Column(children: [
        ...
        if (_isLoading)
          const LinearProgressIndicator(
            ...
            color: Color(0xFF2AABEE), minHeight: 2),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(_cacheSize, style: const TextStyle(
  ...
  Widget _buildEmpty() => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      Container(
        width: 72, height: 72,
        decoration: BoxDecoration(color: const Color(0xFF141420),
      ...
      const Padding(
        ...
        child: Text('No videos, audio or documents found in your chats.',
            textAlign: TextAlign.center,
            style: TextStyle(color: Color(0xFF7070A0), fontSize: 13,
                height: 1.5)),
      ),
      const SizedBox(height: 24),
      OutlinedButton.icon(
  ...
  Widget _buildNoResults() => Center(
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      const Icon(Icons.search_off_rounded, color: Color(0xFF3A3A6A), size: 56),
      const SizedBox(height: 16),
  ...
  Widget _buildFileCard(TelegramFile file) {
    final isLargeVideo =
        file.isVideo && file.fileSize >= kSmallFileThreshold;
        ...
        child: Row(children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Row(children: [
                  ...
                  // Badge: instant download vs streaming
                  const SizedBox(width: 6),
                  _chip(
                    isLargeVideo ? 'Stream' : 'Instant',
                    icon: isLargeVideo
                        ? Icons.wifi_rounded
                        : Icons.bolt_rounded,
                    color: isLargeVideo
          ...
          Container(
            ...
            child: Icon(
              ...
              color: const Color(0xFF2AABEE), size: 22,
            ),
          ),
        ]),
      ),
  ...
  Widget _buildThumbOrIcon(TelegramFile file) {
    ...
    if (file.thumbnail != null && file.thumbnail!.isNotEmpty) {
      return ClipRRect(
        borderRadius: BorderRadius.circular(10),
        child: Image.file(File(file.thumbnail!),
  ...
  Color _colorFor(TelegramFile file) {
    ...
    if (m.contains('zip') || m.contains('rar')) return const Color(0xFFF39C12);
    if (m.contains('word') || m.contains('document')) {
